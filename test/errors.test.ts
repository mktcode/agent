...
describe('shared errors', () => {
  it('provides stable codes and deterministic messages', () => {
    const cases = [
      ...
      [new PostCloneCommandError('npm install', { exitCode: 1 }), 'POST_CLONE_COMMAND_FAILED', 'Post-clone command failed: npm install'],
      [new SessionBusyError(), 'SESSION_BUSY', 'The agent session is already executing a turn.'],
      [new SessionNotFoundError(), 'SESSION_NOT_FOUND', 'The requested agent session was not found.'],
      ...
      [new CurrentBranchMismatchError('feature', 'main'), 'CURRENT_BRANCH_MISMATCH', 'Requested branch feature is not the currently checked out branch main.'],
